...
int main(int argc, char **argv)
{
	...
	ros::ServiceClient rxService = n.serviceClient<rx60controller::command>("command");
	ros::Rate loop_rate(10);
	//int count = 0;
	Path<Q> path;
	rx60controller::command::Request req;
	rx60controller::command::Response resp;
	while (ros::ok())
	{
		int c = getchar();
		if( c == 'a')
		{
			...
			req.command_number = 1;
			bool success = rxService.call(req,resp);
			if(success)
			{
				ROS_INFO_STREAM("Joints1: " << resp.joint1 << " Joints2: " << resp.joint2 << " Joints3: " << resp.joint3 << " Joints4: " << resp.joint4 << " Joints5: " << resp.joint5 << " Joints6: " << resp.joint6);
			}
			else {
				ROS_ERROR_STREAM("get joints failed");
			}
			ros::spinOnce();
